...
public class SOSim_calculate_CAIsRoot_not1 implements CalculateBehavior_hpoTerm_similarity {
    @Override
    public HashMap<String,Double> calculate(HPO_term hpo_term1, HPO_term hpo_term2, TreeMap<String, HPO_term> HPO_terms, int num_diseases)
    {
        ...
        else
        {
            ...
            {
                HashMap<String, Double> temp = calculate(hpo_term1, hpoTerm_MICA, HPO_terms, num_diseases);
                results.put("SOSim", temp.get("SOSim") + (hpo_term2.getD()/num_diseases) - 1);
            }
            else
            {
